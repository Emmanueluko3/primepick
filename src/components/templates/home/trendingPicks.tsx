...
const TrendingPicks: React.FC = () => {
  const trendindPicksitems = products.sort(() => Math.random() - 0.5);
  return (
    <div className="w-full">
      ...
      {trendindPicksitems.length <= 0 ? (
        <h3 className="text-gray-500">
          No Product available, Please create new listing
        </h3>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-5">
          {trendindPicksitems.slice(0, 8).map((item, index) => (
