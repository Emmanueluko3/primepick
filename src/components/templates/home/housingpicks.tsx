...
const HousingPicks: React.FC = () => {
  const housingPicks = products.filter(
  ...
  return (
    <div className="w-full">
      ...
      {housingPicks.length <= 0 ? (
        <h3 className="text-gray-500">
          No House available, Please create new listing
        </h3>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-5">
          {housingPicks.slice(0, 4).map((item, index) => (
